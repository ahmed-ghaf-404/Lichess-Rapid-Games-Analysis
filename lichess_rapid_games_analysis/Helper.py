...
class Helper:
    ...
    def createUsernameCSV(self, username_set: set):
        ...
        df.to_csv(
            "Data/Prepared_Data/Usernames-{}.csv".format(
    ...
    def scrape_n_rapid_games(
        ...
        try:
            gameLists = C.CLIENT.games.export_by_player(
                username=username,
                as_pgn=False,
                max=num_of_games,
                rated=True,
            ...
            for game in gameLists:
                ...
                if os.path.isfile(f"{path}/{username}/{game['id']}.json"):
                    if (
                        os.path.getsize(f"{path}/{username}/{game['id']}.json")
                        == 0
                    ):
                ...
                if is_need_more_players:
                    if (
                        username.lower()
                        == game["players"]["white"]["user"]["name"].lower()
                    ):
                        new_username.add(
    ...
    def validateGame(self, path: str):
        ...
        with open(path, "r") as f:
            ...
            if (
                ...
                or "opening" not in game
                or "provisional" in game["players"]["white"]
                or "provisional" in game["players"]["black"]
                or abs(
                    game["players"]["white"]["rating"]
                    - game["players"]["black"]["rating"]
                )
                > 100
                or len(game["moves"].split(" ")) < 40
            ):
                return False
...
if __name__ == "__main__":
    ...
    print(
        helper.scrape_n_rapid_games(
            "Data/Scraped_Files", "ericrosen", 50, True, True
        )
    )
