...
class DatabaseManager:
    def __init__(
        ...
        try:
            self.userIds = {
                i[0] for i in self.runCommand("SELECT userId from users;")
            }
            self.connectDB()
        ...
        try:
            self.gameIds = {
                i[0] for i in self.runCommand("SELECT gameId from games;")
            }
            self.connectDB()
        except Exception as _:
            self.gameIds = set()
        ...
        try:
            self.gameStatsIds = {
                i[0] for i in self.runCommand("SELECT gameId from gameStats;")
            }
            self.connectDB()
        except Exception as _:
            ...
            for user in os.listdir(path):
                ...
                for game in os.listdir(f"{path}/{user}"):
                    ...
                    with open(path + user + "/" + game) as f:
                        ...
                        users_table_set.add((userId, title, rating))
            print(
                f"Done creating values to add to users table: {len(users_table_set)} element"
            )
    ...
    def populateGamesTable(self, path="./Data/Scraped_Files/"):
        def GetGamesEntry() -> set:
            ...
            print(
                f"Done creating values to add to games table: {len(games_table_set)} element"
            )
            ...
            for user in os.listdir(path):
                ...
                for g in os.listdir(path + user):
                    ...
                    with open(path + user + "/" + g) as f:
                        ...
                        whiteInaccuracy = game["players"]["white"]["analysis"][
                            "inaccuracy"
                        ]
                        whiteMistakes = game["players"]["white"]["analysis"][
                            "mistake"
                        ]
                        whiteBlunder = game["players"]["white"]["analysis"][
                            "blunder"
                        ]
                        whiteACPL = game["players"]["white"]["analysis"][
                            "acpl"
                        ]

                        blackId = game["players"]["black"]["user"]["id"]
                        blackRating = game["players"]["black"]["rating"]
                        blackInaccuracy = game["players"]["black"]["analysis"][
                            "inaccuracy"
                        ]
                        blackMistakes = game["players"]["black"]["analysis"][
                            "mistake"
                        ]
                        blackBlunder = game["players"]["black"]["analysis"][
                            "blunder"
                        ]
                        blackACPL = game["players"]["black"]["analysis"][
                            "acpl"
                        ]
